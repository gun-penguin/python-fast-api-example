...
from service.admin.service.admin_user_service import AdminUserService
...
@router.get("/", response_model=ResponseListModel[AdminUserSchema])
async def get_user_list(page: int = Query(1, ge=1),
    ...
    result.count = await AdminUserService.get_user_count()
    if result.count > 0:
        result.list = await AdminUserService.get_user_list(param)
    return result


@router.get("/{item_id}", response_model=ResponseDataModel[AdminUserSchema])
async def get_user(item_id: int):
    result = ResponseDataModel()
    result.data = await AdminUserService.get_user(item_id)
    return result


@router.post("/", response_model=ResponseModel)
async def create_user(user: AdminUserSchema):
    service_result = await AdminUserService.create_user(user)
